...
use App\Jobs\ProcessDigemidAyudaImport;
    ...
    public function catalog(Request $request)
    {
        ...
        if ($request->hasFile('file')) {
            try {
                ...
                // Guardar el archivo para procesarlo en segundo plano
                $fileName = 'digemid_ayuda_' . time() . '.' . $request->file('file')->getClientOriginalExtension();
                ...
                ProcessDigemidAyudaImport::dispatch($filePath);

                return [
                    'success' => true,
                    'message' =>  __('app.actions.upload.success') . ' (La importación de datos adicionales se está procesando en segundo plano)',
                    'data' => count($updated),
                ];
            } catch (Exception $e) {
                return [
