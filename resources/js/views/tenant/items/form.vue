<template>
    ...
    >
                        ...
                        <div v-show="showPharmaElement" class="col-md-3">
                            ...
                            >
                                ...
                                <div class="input-group">
                                    <el-input
                                        v-model="form.sanitary"
                                    ></el-input>
                                    <div class="input-group-append">
                                        <button
                                            type="button"
                                            class="btn btn-info"
                                            @click="searchByRegSan"
                                            :disabled="!form.sanitary"
                                        >
                    ...
                    <div class="row">
                        ...
                        >
                            <h5 class="separator-title mt-0">
                                ...
                                <small
                                    v-if="
                                        form.item_unit_types.length > 1 &&
                                        !config.enable_list_product
                                    "
                                    class="text-warning"
                                    >Sólo se toma en cuenta el primer registro
                                    para mostrar en POS</small
                                >
                            </h5>
                        </div>
                        <div
        ...
        <!-- Diálogo para mostrar resultados de búsqueda por registro sanitario -->
        <el-dialog
            :visible.sync="dialogRegSanVisible"
            title="Resultados de búsqueda por Registro Sanitario"
            width="80%"
            append-to-body
        >
            <div v-if="regSanResults && regSanResults.length > 0">
                <div class="table-responsive">
                    <table class="table table-hover">
                        <thead>
                            <tr>
                                <th>Código DIGEMID</th>
                                <th>Nombre</th>
                                <th>Descripción</th>
                        ...
                        <tbody>
                            <tr v-for="item in regSanResults" :key="item.id">
                                <td>{{ item.cod_prod }}</td>
                                <td>{{ item.nom_prod }}</td>
                                <td>{{ item.presentac }}</td>
                                <td>{{ item.nom_ifa }}</td>
                                <td>{{ item.concent }}</td>
                                <td>{{ item.num_reg_san }}</td>
                                <td>
                                    <button
                                        type="button"
                                        class="btn btn-success btn-sm"
                                        @click="selectRegSanResult(item)"
                                    >
            ...
            <div v-else class="text-center">
                <p>
                    No se encontraron resultados para el registro sanitario
                    ingresado.
                </p>
            </div>
        </el-dialog>
...
export default {
    ...
    computed: {
        forOnlyShowAllDetails() {
            if (
                this.onlyShowAllDetails != undefined &&
                this.onlyShowAllDetails != null
            )
                return this.onlyShowAllDetails;
    ...
    data() {
        return {
            ...
            // Para el diálogo de búsqueda por registro sanitario
            dialogRegSanVisible: false,
            regSanResults: [],
        };
    },
    async created() {
    ...
    methods: {
        ...
        /**
         * Realiza una búsqueda por registro sanitario
         */
        async searchByRegSan() {
            if (!this.form.sanitary) {
                this.$message.warning(
                    "Debe ingresar un número de registro sanitario para realizar la búsqueda"
                );
                return;
            }
            ...
            try {
                ...
                const response = await this.$http.get(
                    `/digemid/ayuda/search-by-reg-san/${this.form.sanitary}`
                );

                if (response.data.success && response.data.data.length > 0) {
                    this.regSanResults = response.data.data;
                    this.dialogRegSanVisible = true;
                } else {
                    this.$message.info(
                        "No se encontraron resultados para el registro sanitario ingresado"
                    );
                }
            } catch (error) {
                console.error(error);
        ...
        /**
         * Selecciona un resultado de la búsqueda por registro sanitario
         * y completa los campos correspondientes
         *
         * @param {Object} item El registro seleccionado
         */
        selectRegSanResult(item) {
            // Completar campos según las indicaciones
            this.form.description = item.nom_prod; // Nombre
            this.form.name = item.presentac; // Descripción
            this.form.second_name = item.nom_ifa; // Nombre secundario
            this.form.cod_digemid = item.cod_prod; // Código DIGEMID

            // Cerrar el diálogo
            this.dialogRegSanVisible = false;

            this.$message.success("Datos completados correctamente");
        },
    },
};
</script>
